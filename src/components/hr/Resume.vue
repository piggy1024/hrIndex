          ...
          <template slot-scope="scope">
            <el-dropdown size="mini" split-button type="primary">
              ...
              <el-dropdown-menu>
                ...
                <el-dropdown-item @click.native="downloadResume(scope.row.applicationId)">下载简历</el-dropdown-item>
                <el-dropdown-item @click.native="shareResume(scope.row.applicationId)">推荐/共享</el-dropdown-item>
...
export default {
  ...
  methods: {
    ...
    handleCommand(position) {
      ...
      let params1 = position;
      // console.log(params1);

      hrApi.getRequest1(url, params1).then(res => {
        console.log(res);
      });
    ...
    searchKeyword(keyword) {
      ...
      hrApi.postRequest(url, params).then(res => {
        this.tableData = res.data.posInfo;
      });
    },
    // hr通知发送offer
    dialogSendofferConfirm() {
      ...
      this.$refs.formSendoffer.validate(valid => {
        /*会自动根据rules进行校验*/
        if (valid) {
          console.log(this.formSendoffer.description);

          // url 为接口网址
          let url = "/sendoffer/" + this.applicationId;
          let params = "interviewsDesc=" + this.formSendoffer.description;
